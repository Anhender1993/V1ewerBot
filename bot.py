...
# Slash Command: /liststreamers (Everyone)
...
# Slash Command: /checkstreamers (Admin Only, Manual Twitch Check)
@bot.tree.command(name="checkstreamers", description="Manually check if any tracked streamers are live", guild=discord.Object(id=GUILD_ID))
async def manual_check_streamers(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message("❌ You don't have permission to use this command.", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.followup.send(f"Error reading streamers list: {e}")
        return
    ...
    await interaction.followup.send(f"✅ Manual check complete! Announced {announcements} new live stream(s).")
